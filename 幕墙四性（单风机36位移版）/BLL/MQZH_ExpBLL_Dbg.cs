    ...
    public partial class MQZH_ExpBLL : ObservableObject
    {
        ...
        private ushort SDQMFunc()
        {
            try
            {
                ...
                double tempNow;
                //根据给定值的范围选择实测值
                if ((SDQMDoubleValues[8] >= BllDev.AIList[0].SingalLowerRange * 0.9) && (SDQMDoubleValues[8] <= BllDev.AIList[0].SingalUpperRange * 0.9))
                {
                    tempNow = BllDev.AIList[0].ValueFinal;
                }
                else
                {
                    tempNow = BllDev.AIList[2].ValueFinal;
                }
        ...
        private ushort SDSMFunc()
        {
            try
            {
                ...
                double tempNow = 0;
                //根据给定值的范围选择实测值
                if (BllDev.IsWithCYM && (SDSM1DoubleValues[8] >= BllDev.AIList[1].SingalLowerRange * 0.9) && (SDSM1DoubleValues[8] < BllDev.AIList[1].SingalUpperRange * 0.9))
                    tempNow = BllDev.AIList[1].ValueFinal;
                else
                    tempNow = BllDev.AIList[2].ValueFinal;
                double tempErr = tempGiven - tempNow;
